...
def conjunto_difuso(x,a,b,c):
	...
	if x < a: # Si el valor de entrada es menor que el límite inferior.
        ...
        return (x - a) / (b - a)  # Calculamos el grado de pertenencia usando función trapezoidal.
...
def control_difuso(temperatura, humedad):
	...
	frio = (0, 15, 0)  # Definir el conjunto difuso "frío" con límites inferior y superior y pendiente.
	...
	temperatura_frio = conjunto_difuso(temperatura, *frio)  # Calcular grado de pertenencia al conjunto "frío".
    temperatura_templado = conjunto_difuso(temperatura, *templado)  # Calcular grado de pertenencia al conjunto "templado".
    temperatura_caliente = conjunto_difuso(temperatura, *caliente)  # Calcular grado de pertenencia al conjunto "caliente".

    humedad_seco = conjunto_difuso(humedad, *seco)  # Calcular grado de pertenencia al conjunto "seco".
    humedad_normal = conjunto_difuso(humedad, *normal)  # Calcular grado de pertenencia al conjunto "normal".
    humedad_humedo = conjunto_difuso(humedad, *humedo)  # Calcular grado de pertenencia al conjunto "húmedo".
	...
	# Recordar que la multiplicación entre matrices no es conmutativa.
	 reglas_difusas = np.array([humedad_seco, humedad_normal, humedad_humedo])  # Crear matriz de reglas difusas para la humedad.
    peso_temperatura = np.array([temperatura_frio, temperatura_templado, temperatura_caliente])  # Crear matriz de pesos de temperatura.
    salida = np.dot(reglas_difusas, peso_temperatura)  # Realizar multiplicación matricial para obtener salida difusa.
...
def control_difuso(temperatura, humedad):
    ...
    peso_temperatura = np.array([temperatura_frio, temperatura_templado, temperatura_caliente])
