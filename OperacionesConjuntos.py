...
# Define a function to calculate the set of integers from a given formula
def calculate_set(formula, range_start, range_end):
    result_set = set()
    for x in range(range_start, range_end + 1):
        r = formula(x)
        if r.is_integer() and 1 <= r <= 29:
            result_set.add(int(r))
    return result_set

# Define the formulas for the sets
formula_A = lambda x: (3 * x + 1) / 2
formula_B = lambda n: (n**2 - 4) / (n - 2) if n != 2 else 0  # Avoid division by zero
formula_C = lambda n: (5 * n + 1) / 3

# Calculate the sets A, B, and C using the defined formulas
A = calculate_set(formula_A, 2, 29)
B = calculate_set(formula_B, 1, 18)
C = calculate_set(formula_C, 3, 19)
